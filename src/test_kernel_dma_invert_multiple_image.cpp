...
int main()
{
	...
	for (int try_n = 0; try_n < TRIES_N; try_n++)
	{
		...
		for (int i = 0; i < LENGTH_OUTPUT; i++) {
			if (out_buff[i] != inp_buff[i*3]*R_Weight + inp_buff[(i*3) + 1]*G_Weight + inp_buff[(i*3) + 2]*B_Weight) {
				printf("\nFailure in out_buff: %i %d != %d --addr: %x\n\r", i, out_buff[i], inp_buff[i*3]*R_Weight + inp_buff[(i*3) + 1]*G_Weight + inp_buff[(i*3) + 2]*B_Weight, &out_buff[i]);
				FAIL = true;
				break;
			}
